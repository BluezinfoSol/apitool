...
import useCave7Store from '@/src/Cave7Store';


export interface Cave7Props {
}

export default function Cave7 (props: Cave7Props) {
  ...
  useEffect(() => {
    const intervalId = setInterval(async () => {
      for (const order of orderdata) {
        const codeResponse = await axios.get('/api/cave7/getCode', {
          params: {
            apiKey: apiKey,
  ...
  const handleGetNumberClick = async () => {
    try {
        ...
        const response = await axios.post('/api/cave7', {
            apiKey,
            service,
            country,
            operator,
  ...
  const handleNumberClick = async (orderId: string, status: string) => {
    const codeResponse = await axios.post('/api/cave7/getCode', {
      apiKey: apiKey,
      orderId: orderId,
      status: status
    }).then((res)=> {
