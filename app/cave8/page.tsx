...
import useCave8Store from '@/src/Cave8Store';


export interface Cave8Props {
}

export default function Cave8 (props: Cave8Props) {
  ...
  useEffect(() => {
    const intervalId = setInterval(async () => {
      for (const order of orderdata) {
        const codeResponse = await axios.get('/api/cave8/getCode', {
          params: {
            apiKey: apiKey,
  ...
  const handleGetNumberClick = async () => {
    try {
        ...
        const response = await axios.post('/api/cave8', {
            apiKey,
            service,
            country,
            operator,
  ...
  const handleNumberClick = async (orderId: string, status: string) => {
    const codeResponse = await axios.post('/api/cave8/getCode', {
      apiKey: apiKey,
      orderId: orderId,
      status: status
    }).then((res)=> {
