...
export async function GET(request: NextRequest) {
    ...
    const response = await fetch(`https://smshub.org/stubs/handler_api.php?api_key=${apiKey}&action=getBalance`);
...
export async function POST(request: NextRequest) {
    try {
        ...
        const response = await fetch(`https://smshub.org/stubs/handler_api.php?api_key=${req.apiKey}&action=getNumber&service=${req.service}&operator=any&country=${req.country}`);
        if (!response.ok) {
